...
You can execute the `ListAssemblyInstructionsForProduct` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useListAssemblyInstructionsForProduct(dc: DataConnect, vars: ListAssemblyInstructionsForProductVariables, options?: useDataConnectQueryOptions<ListAssemblyInstructionsForProductData>): UseDataConnectQueryResult<ListAssemblyInstructionsForProductData, ListAssemblyInstructionsForProductVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListAssemblyInstructionsForProduct(vars: ListAssemblyInstructionsForProductVariables, options?: useDataConnectQueryOptions<ListAssemblyInstructionsForProductData>): UseDataConnectQueryResult<ListAssemblyInstructionsForProductData, ListAssemblyInstructionsForProductVariables>;
```
...
```
### Return Type
Recall that calling the `ListAssemblyInstructionsForProduct` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListAssemblyInstructionsForProduct` Query is of type `ListAssemblyInstructionsForProductData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListAssemblyInstructionsForProduct`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, ListAssemblyInstructionsForProductVariables } from '@dataconnect/generated';
import { useListAssemblyInstructionsForProduct } from '@dataconnect/generated/react'

export default function ListAssemblyInstructionsForProductComponent() {
  // The `useListAssemblyInstructionsForProduct` Query hook requires an argument of type `ListAssemblyInstructionsForProductVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useListAssemblyInstructionsForProduct(listAssemblyInstructionsForProductVars);
  // Variables can be defined inline as well.
  const query = useListAssemblyInstructionsForProduct({ productId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListAssemblyInstructionsForProduct(dataConnect, listAssemblyInstructionsForProductVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useListAssemblyInstructionsForProduct(listAssemblyInstructionsForProductVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useListAssemblyInstructionsForProduct(dataConnect, listAssemblyInstructionsForProductVars, options);
  ...
  // If the Query is successful, you can access the data returned using the `UseQueryResult.data` field.
  if (query.isSuccess) {
    console.log(query.data.assemblyInstructions);
  }
  return <div>Query execution {query.isSuccess ? 'successful' : 'failed'}!</div>;
}
```
